...
async def play_stream_router(client: Client, message: Message):
    ...
    if len(message.command) < 2:
        await message.edit("❌ **Usage Format:** `.play [Track Name / YouTube URL]`")
        return

    search_query = message.text.split(None, 1)[1]
    tracking_panel = await message.edit("🔍 **Searching Audio...**")
    ...
    group_music_queue[chat_id].append({
        ...
        "audio": audio_node,
        "duration": track_len,
        "author": message.from_user.mention if message.from_user else "Owner",
...
async def music_dashboard_callback(client: Client, callback_query: CallbackQuery):
    ...
    if target_chat not in group_music_queue or not group_music_queue[target_chat]:
        await callback_query.answer("❌ No active playlist found.", show_alert=True)
        return
    ...
    if control_action == "pause":
        ...
        await callback_query.message.edit("⏹️ **Stream session terminated.**")
